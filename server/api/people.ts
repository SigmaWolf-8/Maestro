...
import { z } from "zod";
import { type Customer, type VendorContact, type CustomerContact, type TenantUser } from "@shared/schema";
...
const customerContactCreateSchema = z.object({
  tenantId: z.string(),
  customerId: z.string(),
  firstName: z.string().optional(),
  lastName: z.string().optional(),
...
const customerContactUpdateSchema = customerContactCreateSchema.partial().omit({ tenantId: true, customerId: true });
...
export function createPeopleRouter(): Router {
  ...
  router.get("/api/customers/:customerId/contacts", async (req, res) => {
    try {
  ...
  router.post("/api/customers/:customerId/contacts", validateBody(customerContactCreateSchema.omit({ customerId: true })), async (req, res) => {
    try {
  ...
  router.patch("/api/customer-contacts/:id", validateBody(customerContactUpdateSchema), async (req, res) => {
    try {
  ...
  router.delete("/api/customer-contacts/:id", async (req, res) => {
    try {
  ...
  router.get("/api/contacts/directory", async (req, res) => {
    try {
      ...
      const groupByRaw = (req.query.groupBy as string) || "";
      const groupByFields = groupByRaw ? groupByRaw.split(",").filter(Boolean) : [];
      ...
      const customerContacts = customers.map((c: Customer) => ({
        ...
        lastName: c.lastName || "",
        company: "",
        jobNumber: c.jobNum ? `${c.jobNum}` : "",
        email: c.email1 || c.email2 || "",
        phone: formatPhone(c.mobilePhone || c.workPhone || c.homePhone),
        jobTitle: "",
        city: c.city || "",
        sourceId: c.id,
      }));
      ...
      const vendorContactsList = vendorContacts.map((vc: { contact: VendorContact; vendorName: string }) => ({
        ...
        phone: formatPhone(vc.contact.businessPhone || vc.contact.mobilePhone),
      ...
      if (search) {
        ...
        allContacts = allContacts.filter(c => 
          ...
          c.jobNumber.includes(search)
        );
      }
      
      const dir = sortDirection === "desc" ? -1 : 1;
      
      const fieldCompare = (a: any, b: any, field: string): number => {
        switch (field) {
          ...
          case "jobNumber": return (parseInt(a.jobNumber) || 0) - (parseInt(b.jobNumber) || 0);
          ...
          default: return a.lastName.localeCompare(b.lastName) || a.firstName.localeCompare(b.firstName);
        }
      };
      ...
      allContacts.sort((a, b) => {
        if (groupByFields.length > 0) {
          for (const gf of groupByFields) {
            const gcmp = fieldCompare(a, b, gf);
            if (gcmp !== 0) return gcmp;
          }
        }
        const cmp = fieldCompare(a, b, sortBy);
        return cmp * dir;
      });
