...
class DocumentService {
  async upload(options: DocumentUploadOptions): Promise<DocumentUploadResult> {
    ...
    try {
      const tsResult = await kongService.getTimestamp();
      kongTimestamp = tsResult.timestamp.humanReadable;
    } catch {
      // Kong unavailable — non-fatal
    }
    ...
    if (sanitizedContent && options.encrypt) {
      try {
        ...
        encryptedContent = JSON.stringify(encResult.encrypted);
  ...
  async reEncrypt(
    ...
    const encResult = await kongService.encryptData(content.content, newMode);
    const encryptedContent = JSON.stringify(encResult.encrypted);
    const originalSize = Buffer.byteLength(content.content, "utf8");
    const compressedSize = encResult.encryptedSize;
