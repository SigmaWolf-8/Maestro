...
export interface IStorage {
  ...
  // Tenant Applications
  getTenantApplications(tenantId: string): Promise<TenantApplication[]>;
  createTenantApplication(app: InsertTenantApplication): Promise<TenantApplication>;
  deleteTenantApplication(id: string): Promise<boolean>;
  ...
  // Customer Contacts (multiple contacts per customer/job)
  getCustomerContacts(customerId: string): Promise<CustomerContact[]>;
  getAllCustomerContacts(tenantId: string): Promise<{ contact: CustomerContact; jobNum: number }[]>;
  getCustomerContact(id: string): Promise<CustomerContact | undefined>;
  getPrimaryCustomerContact(customerId: string): Promise<CustomerContact | undefined>;
  createCustomerContact(contact: InsertCustomerContact): Promise<CustomerContact>;
  updateCustomerContact(id: string, updates: Partial<CustomerContact>): Promise<CustomerContact | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Tenant Applications
  async getTenantApplications(tenantId: string): Promise<TenantApplication[]> {
    return db.select().from(tenantApplications)
      .where(eq(tenantApplications.tenantId, tenantId))
      .orderBy(tenantApplications.itemOrder);
  }

  async createTenantApplication(app: InsertTenantApplication): Promise<TenantApplication> {
    const id = randomUUID();
    const [created] = await db.insert(tenantApplications).values({ ...app, id }).returning();
    return created;
  }

  async deleteTenantApplication(id: string): Promise<boolean> {
    await db.delete(tenantApplications).where(eq(tenantApplications.id, id));
    return true;
  }
  ...
  // Customer Contacts implementation
  ...
  async getAllCustomerContacts(tenantId: string): Promise<{ contact: CustomerContact; jobNum: number }[]> {
    const results = await db
      .select({
        contact: customerContacts,
        jobNum: customers.jobNum,
      })
  ...
  async getPrimaryCustomerContact(customerId: string): Promise<CustomerContact | undefined> {
    const [contact] = await db.select().from(customerContacts).where(
  ...
  async createCustomerContact(contact: InsertCustomerContact): Promise<CustomerContact> {
    const id = randomUUID();
    const [created] = await db.insert(customerContacts).values({ ...contact, id }).returning();
    return created;
  }

  async updateCustomerContact(id: string, updates: Partial<CustomerContact>): Promise<CustomerContact | undefined> {
    const [updated] = await db.update(customerContacts).set({ ...updates, updatedAt: new Date() }).where(eq(customerContacts.id, id)).returning();
    return updated || undefined;
  }
...
import { createEncryptedStorage } from "./security/encrypted-storage-wrapper";

export const storage = createEncryptedStorage(new DatabaseStorage());
...
export async function seedNavigationForTenant(tenantId: string) {
  ...
  await db.insert(navigationItems).values([
    { id: randomUUID(), tenantId, parentId: null, itemOrder: 75, itemType: "menu", title: "Applications", iconName: "AppWindow", path: "/applications", maxChildrenDisplay: 10, isCollapsible: true, minRoleRequired: "viewer", createdAt: new Date(), updatedAt: new Date() },
  ]);
}
...
function getNavigationTemplate(companyType: string): NavSection[] {
  ...
  const applications: NavSection = {
    title: "Applications", iconName: "AppWindow", order: 75, minRole: "viewer", maxChildren: 10,
    children: [],
  };
